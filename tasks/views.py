    ...
    if request.method == 'GET':
        ...
        if request.POST['password1'] == request.POST['password2']:
            try:
                ...
                user.save()
                ## Crear id unica para validar el inicio de sesión
                login(request, user)
...
def tasks(request):
    ...
    ## filtramos que las tareas ssean sólo de un usuario con el filter y mostrar sólo las que no ha completado
...
def create_task(request):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            ## Para no guardarlo en la base de datos, solo que devuelva los datos
...
def task_detail(request,task_id):
    if request.method == 'GET':
        ## pk == primary key y manejamos el error
        task = get_object_or_404(Tasks, pk = task_id, user = request.user)
        ## Modificar tarea creada
